...
public final class RotationUtils
{
    ...
    public static Optional<Rotation> reachable(final IPlayerContext playerContext, final et et, final boolean b) {
        return reachable(playerContext.player(), et, playerContext.playerController().getBlockReachDistance(), b);
    }
    
    public static Optional<Rotation> reachable(final bud bud, final et et, final double n) {
        return reachable(bud, et, n, false);
    }
    
    public static Optional<Rotation> reachable(final bud bud, final et et, final double n, final boolean b) {
        if (BaritoneAPI.getProvider().getBaritoneForPlayer(bud).getPlayerContext().isLookingAt(et)) {
            final Rotation rotation = new Rotation(bud.v, bud.w + 1.0E-4f);
            if (!b) {
                return Optional.of(rotation);
            }
            final bhc rayTraceTowards;
            if ((rayTraceTowards = RayTraceUtils.rayTraceTowards((vg)bud, rotation, n, true)) != null && rayTraceTowards.a == bhc$a.b && rayTraceTowards.a().equals((Object)et)) {
                return Optional.of(rotation);
            }
        }
        final Optional<Rotation> reachableCenter;
        if ((reachableCenter = reachableCenter((vg)bud, et, n, b)).isPresent()) {
        ...
        for (int length = (block_SIDE_MULTIPLIERS = RotationUtils.BLOCK_SIDE_MULTIPLIERS).length, i = 0; i < length; ++i) {
            ...
            if ((reachableOffset = reachableOffset((vg)bud, et, new bhe((fq)et).b(e.a * bhe.b + e.d * (1.0 - bhe.b), e.b * bhe.c + e.e * (1.0 - bhe.c), e.c * bhe.d + e.f * (1.0 - bhe.d)), n, b)).isPresent()) {
    ...
    public static Optional<Rotation> reachableOffset(final vg vg, final et et, final bhe bhe, final double n, final boolean b) {
        final Rotation calcRotationFromVec3d = calcRotationFromVec3d(b ? RayTraceUtils.inferSneakingEyePosition(vg) : vg.f(1.0f), bhe, new Rotation(vg.v, vg.w));
        final bhc rayTraceTowards;
        if ((rayTraceTowards = RayTraceUtils.rayTraceTowards(vg, calcRotationFromVec3d, n, b)) != null && rayTraceTowards.a == bhc$a.b) {
    ...
    public static Optional<Rotation> reachableCenter(final vg vg, final et et, final double n, final boolean b) {
        return reachableOffset(vg, et, VecUtils.calculateBlockCenter(vg.l, et), n, b);
    }
