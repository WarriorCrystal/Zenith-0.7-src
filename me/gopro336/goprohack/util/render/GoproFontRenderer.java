...
package me.gopro336.goprohack.util.render;
...
public class GoproFontRenderer
{
    private static final Pattern COLOR_CODE_PATTERN;
    public final int FONT_HEIGHT = 9;
    public final int FONT_WATERMARK_HEIGHT = 18;
    ...
    public GoproFontRenderer(final String fontName, final float fontSize) {
        ...
        try {
            ...
            (this.unicodeFont = new UnicodeFont(getFontByName(fontName).deriveFont(fontSize * this.prevScaleFactor / 2.0f))).addAsciiGlyphs();
            ...
            this.prevScaleFactor = resolution.getScaleFactor();
            try {
                (this.unicodeFont = new UnicodeFont(getFontByName("Tw Cen MT").deriveFont(fontSize * this.prevScaleFactor / 2.0f))).addAsciiGlyphs();
                this.unicodeFont.getEffects().add(new ColorEffect(Color.WHITE));
                this.unicodeFont.loadGlyphs();
            }
            catch (Exception e2) {
    ...
    public GoproFontRenderer(final Font font) {
        this(font.getFontName(), (float)font.getSize());
    }
    
    public GoproFontRenderer(final String fontName, final int fontType, final int size) {
    ...
    public static Font getFontByName(final String name) throws IOException, FontFormatException {
        if (name == "Tw Cen MT") {
            return getFontFromInput("/assets/goprohack/fonts/TwCenMT-Bold-Italic.ttf");
        }
        if (name == "TwCenMT-Bold-Italic") {
            return getFontFromInput("/assets/goprohack/fonts/TwCenMT-Bold-Italic.ttf");
        }
        if (name == "Comfortaa-regular") {
            return getFontFromInput("/assets/goprohack/fonts/Comfortaa-regular.ttf");
        }
        if (name == "Tw Cen MT Std") {
            return getFontFromInput("/assets/goprohack/fonts/TwCenMTStd-Bold.ttf");
        }
        if (name == "Verdana Bold") {
            return getFontFromInput("/assets/goprohack/fonts/verdanabold.ttf");
        }
        if (name == "Lucida Console") {
            return getFontFromInput("/assets/goprohack/fonts/lucdia-console.ttf");
        }
        if (name == "VerdanaBold") {
            return getFontFromInput("/assets/goprohack/fonts/verdanabold.ttf");
        }
        return Font.createFont(0, new File(DirectoryManager.Get().GetCurrentDirectory() + "\\GoproHack\\Fonts\\" + name + ".ttf"));
    }
    
    public static Font getFontFromInput(final String path) throws IOException, FontFormatException {
        final Font newFont = Font.createFont(0, GoproFontRenderer.class.getResourceAsStream(path));
        if (newFont != null) {
            GoproFontRenderer.font = newFont;
        }
        return GoproFontRenderer.font;
    }
    ...
    public int drawString(final String text, float x, float y, final int color) {
        ...
        try {
            if (resolution.getScaleFactor() != this.prevScaleFactor) {
                this.prevScaleFactor = resolution.getScaleFactor();
                (this.unicodeFont = new UnicodeFont(getFontByName(this.name).deriveFont(this.size * this.prevScaleFactor / 2.0f))).addAsciiGlyphs();
                this.unicodeFont.getEffects().add(new ColorEffect(Color.WHITE));
                this.unicodeFont.loadGlyphs();
            }
        }
        ...
        final String[] parts = GoproFontRenderer.COLOR_CODE_PATTERN.split(text);
        int index = 0;
    ...
    public float getStringWidth(final String p_Name) {
        return (float)(this.unicodeFont.getWidth(ChatColor.stripColor(p_Name)) / 2);
    }
    ...
    public String trimStringToWidth(final String text, final int width, final boolean reverse) {
        ...
        for (int l = j; l >= 0 && l < text.length() && i < width; l += k) {
            ...
            final float i2 = this.getWidth(text);
            if (flag) {
                flag = false;
                if (c0 != 'l' && c0 != 'L') {
                    if (c0 == 'r' || c0 == 'R') {
