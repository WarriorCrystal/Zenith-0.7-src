...
public class BlockInteractionHelper
{
    ...
    public static boolean checkForNeighbours(final BlockPos blockPos) {
        if (!hasNeighbour(blockPos)) {
            for (final EnumFacing side : EnumFacing.values()) {
                ...
                if (side == EnumFacing.UP && BlockInteractionHelper.mc.world.getBlockState(blockPos).getBlock() == Blocks.WATER && BlockInteractionHelper.mc.world.getBlockState(blockPos.up()).getBlock() == Blocks.AIR && ModuleManager.Get().GetMod(LiquidInteractModule.class).isEnabled()) {
                    return true;
                }
            }
            return false;
    ...
    public static ValidResult valid(final BlockPos pos) {
        ...
        if (BlockInteractionHelper.mc.world.getBlockState(pos.down()).getBlock() == Blocks.WATER && ModuleManager.Get().GetMod(LiquidInteractModule.class).isEnabled()) {
            ...
            for (final BlockPos l_Pos : array) {
                ...
                if (l_State2.getBlock() != Blocks.AIR) {
                    for (final EnumFacing side : EnumFacing.values()) {
                        ...
                        if (BlockInteractionHelper.mc.world.getBlockState(neighbor).getBlock().canCollideCheck(BlockInteractionHelper.mc.world.getBlockState(neighbor), false) || (l_IsWater && ModuleManager.Get().GetMod(LiquidInteractModule.class).isEnabled())) {
                            return ValidResult.Ok;
                        }
                    }
                }
