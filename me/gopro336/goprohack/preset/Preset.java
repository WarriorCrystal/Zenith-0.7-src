...
package me.gopro336.goprohack.preset;
...
public class Preset
{
    ...
    public void load(final File directory) {
        final File exists = new File("GoproHack/Presets/" + directory.getName() + "/" + directory.getName() + ".json");
        if (!exists.exists()) {
            return;
        }
        try {
            final Gson gson = new Gson();
            final Reader reader = Files.newBufferedReader(Paths.get("GoproHack/Presets/" + directory.getName() + "/" + directory.getName() + ".json", new String[0]));
        ...
        try (final Stream<Path> paths = Files.walk(Paths.get("GoproHack/Presets/" + directory.getName() + "/Modules/", new String[0]), new FileVisitOption[0])) {
            Gson gson2;
            Reader reader2;
            Map<?, ?> map2;
            ...
            paths.filter(x$0 -> Files.isRegularFile(x$0, new LinkOption[0])).forEach(path -> {
                try {
                    gson2 = new Gson();
                    reader2 = Files.newBufferedReader(Paths.get("GoproHack/Presets/" + directory.getName() + "/Modules/" + path.getFileName().toString(), new String[0]));
    ...
    public void save() {
        try {
            ...
            final Writer writer = Files.newBufferedWriter(Paths.get("GoproHack/Presets/" + this._displayName + "/" + this._displayName + ".json", new String[0]), new OpenOption[0]);
        ...
        try {
            for (final Map.Entry<String, ConcurrentHashMap<String, String>> entry : this._valueListMods.entrySet()) {
                ...
                final Writer writer2 = Files.newBufferedWriter(Paths.get("GoproHack/Presets/" + this._displayName + "/Modules/" + entry.getKey() + ".json", new String[0]), new OpenOption[0]);
                        ...
                        if (l_Val.getName().equalsIgnoreCase(value.getKey())) {
                            if (l_Val.getValue() instanceof Number && !(l_Val.getValue() instanceof Enum)) {
                                ...
                                if (l_Val.getValue() instanceof Enum) {
                                    final Enum e = l_Val.GetEnumReal(l_Value);
                                    if (e != null) {
