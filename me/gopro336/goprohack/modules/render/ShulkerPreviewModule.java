...
public final class ShulkerPreviewModule extends Module
{
    ...
    public ShulkerPreviewModule() {
        super("ShulkerPeek", new String[] { "SPreview", "ShulkerView" }, "Hover over a shulker box to the items inside.", "NONE", 14367780, ModuleType.RENDER);
        ...
        this.Mode = new Value<Modes>("Mode", new String[] { "Server Mode" }, "Server mode, like 2b2t has custom preview settings", Modes.DropPacket);
        ...
        this.OnEntityJoinWorld = new Listener<EntityJoinWorldEvent>(p_Event -> {
            if (this.Mode.getValue() != Modes.DropPacket) {
                return;
            }
                ...
                this.timer.schedule(new TimerTask() {
                    ...
                    public void run() {
                        ...
                        final NBTTagCompound shulkerNBT = ShulkerPreviewModule.this.getShulkerNBT(shulker);
                        if (shulkerNBT != null) {
                            ...
                            if (ShulkerPreviewModule.this.SavedShulkerItems.containsKey(customName)) {
                                ShulkerPreviewModule.this.SavedShulkerItems.remove(customName);
                            }
                            else {
                                GoproHack.SendMessage("New shulker found with name " + customName + " it contains " + l_SlotsNotEmpty + " slots NOT empty");
                            }
                            ShulkerPreviewModule.this.SavedShulkerItems.put(customName, l_Items);
                        }
                    }
                }, 5000L);
                return;
        ...
        this.PacketEvent = new Listener<EventNetworkPacketEvent>(p_Event -> {
            if (p_Event.getPacket() instanceof SPacketWindowItems) {
                ...
                l_Packet3 = (SPacketSetSlot)p_Event.getPacket();
                if (l_Packet3.getWindowId() == this.EnderChestWindowId) {
                    GoproHack.SendMessage("Ender Chest updated, reopen to update preview.");
                }
            }
            return;
        ...
        this.OnRenderTooltip = new Listener<EventRenderTooltip>(p_Event -> {
            if (p_Event.getItemStack() != null) {
                ...
                if (Item.getIdFromItem(p_Event.getItemStack().getItem()) == 130) {
                    ...
                    RenderUtil.drawStringWithShadow("GoproHack EnderChest Viewer", 0.0f, (float)(-mc.fontRenderer.FONT_HEIGHT - 1), -1);
                    ...
                    if (this.middleClick.getValue()) {
                        if (Mouse.isButtonDown(2)) {
                            if (!this.clicked) {
                                l_Inventory = new InventoryBasic("GoproHack EnderChest Viewer", true, 27);
                    ...
                    else if (this.Mode.getValue() == Modes.DropPacket || this.Mode.getValue() == Modes.Inventory) {
                        this.Render2b2tShulkerPreview(p_Event);
                    }
                }
            }
        }, (Predicate<EventRenderTooltip>[])new Predicate[0]);
    }
